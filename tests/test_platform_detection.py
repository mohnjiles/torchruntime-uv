import pytest
from torch_runtime.platform_detection import get_torch_platform, AMD, NVIDIA, INTEL, os_name, arch, py_version


def test_no_discrete_gpus_windows(monkeypatch):
    monkeypatch.setattr("torch_runtime.platform_detection.os_name", "Windows")
    monkeypatch.setattr("torch_runtime.platform_detection.arch", "amd64")
    assert get_torch_platform([]) == "cpu"


def test_no_discrete_gpus_linux(monkeypatch):
    monkeypatch.setattr("torch_runtime.platform_detection.os_name", "Linux")
    monkeypatch.setattr("torch_runtime.platform_detection.arch", "x86_64")
    assert get_torch_platform([]) == "cpu"


def test_no_discrete_gpus_mac(monkeypatch):
    monkeypatch.setattr("torch_runtime.platform_detection.os_name", "Darwin")
    monkeypatch.setattr("torch_runtime.platform_detection.arch", "arm64")
    assert get_torch_platform([]) == "cpu"
...
def test_unsupported_architecture(monkeypatch):
    monkeypatch.setattr("torch_runtime.platform_detection.os_name", "Linux")
    monkeypatch.setattr("torch_runtime.platform_detection.arch", "sparc")
    with pytest.raises(NotImplementedError):
        get_torch_platform([])
